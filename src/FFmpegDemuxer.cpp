...
{
  /*
  这个函数主要负责从FFmpeg格式上下文中读取视频帧，
  处理SEI数据，并准备视频数据和包属性供解码器使用。
  它使用了FFmpeg库的一些函数，如av_read_frame、av_packet_unref、av_bsf_send_packet等
   */
  if (!fmtc) {
    return false;
  }
  ...
  auto appendBytes = [](vector<uint8_t>& elementaryBytes, AVPacket& avPacket,
    ...
    if (isFilteringNeeded) {
      ...
      /*
      这两个函数通常在 FFmpeg 的编码、解码或转码过程中使用。
      例如，当需要对视频帧进行编码前的处理时，可以通过 av_bsf_send_packet
      发送原始帧给过滤器，处理完成后，使用 av_bsf_receive_packet
      获取处理后的帧。
      */
